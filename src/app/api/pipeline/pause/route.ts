...
export async function POST(req: NextRequest) {
  try {
    ...
    const pipeline = await prisma.pipeline.upsert({
      ...
      update: { status: 'paused', updatedAt: new Date() },
      create: { countryId: country.id, status: 'paused', clicks: 0, conversions: 0 },
      include: { country: { select: { code: true, name: true } } },
    });

    return NextResponse.json({
      ok: true,
      message: `Pipeline paused for ${country.name}`,
      pipeline: { id: pipeline.id, country: pipeline.country, status: pipeline.status },
    });
  } catch (err) {
    console.error('[pipeline/pause]', err);
